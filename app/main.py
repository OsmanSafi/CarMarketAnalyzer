...
from app.services.trade_valuation_service import (
    build_trade_valuation,
)
...
def compare_values(
    ...
    difference_percent = difference / internal_value * 100
...
async def get_car(
    ...
    analysis = analyze_market(
        ...
        subject_year=year,
        subject_trim=trim,
        subject_state=normalized_state,
        asking_price=asking_price,
        subject_days_on_market=(days_on_market),
    )
...
async def get_valuation(
    ...
    comparable_market_value = analysis.get("comparable_market_value")
    ...
    valuation_consensus = build_valuation_consensus(
        internal_value=(comparable_market_value),
    ...
    modeled_acquisition = None

    if dealer_economics is not None:
        modeled_acquisition = dealer_economics.get("modeled_dealer_acquisition")

    trade_valuation = build_trade_valuation(
        modeled_dealer_acquisition=(modeled_acquisition),
        carsxe_wholesale_value=(wholesale_value),
        carmax_offer=carmax_offer,
        carvana_offer=carvana_offer,
    ...
    return {
        ...
        "trade_valuation": (trade_valuation),
